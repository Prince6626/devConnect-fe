    ...
    return (
      <div className="min-h-screen bg-[#121212] flex items-center justify-center p-8">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-white mb-4">No Requests Yet</h1>
          <p className="text-gray-400 text-lg">When developers want to connect with you, their requests will appear here!</p>
        </motion.div>
      </div>
    );
  }
      ...
      >
        ...
        <div className="text-center mb-12">
          ...
          >
            ...
            <span className="text-gray-300 font-medium">{requests.length} Pending Requests</span>
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {requests.map((request, index) => {
            const { _id, firstName, lastName, about, photoUrl, skills } = request.fromUserId;

            return (
              <motion.div
                key={_id}
                className="bg-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-gray-700/50 shadow-xl hover:shadow-2xl transition-all duration-300 hover:border-gray-600/50 group"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ 
                  duration: 0.5, 
              ...
              >
                ...
                {skills && skills.length > 0 && (
                  <div className="space-y-3 mb-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-gray-300 font-medium text-sm">Skills</span>
                    </div>
                ...
                <div className="flex gap-3 mt-6">
                  <motion.button
                    className="flex-1 bg-red-600/20 hover:bg-red-600/30 border border-red-500/50 hover:border-red-400/70 text-red-400 hover:text-red-300 font-medium py-3 px-4 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 backdrop-blur-sm"
                  ...
                  <motion.button
                    className="flex-1 bg-green-600/20 hover:bg-green-600/30 border border-green-500/50 hover:border-green-400/70 text-green-400 hover:text-green-300 font-medium py-3 px-4 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 backdrop-blur-sm"
